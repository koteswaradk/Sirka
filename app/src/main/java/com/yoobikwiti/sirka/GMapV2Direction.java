...
public class GMapV2Direction {
            ...
            for (int i = 0; i < nl1.getLength(); i++) {
                ...
                for (int j = 0; j < arr.size(); j++) {
                    listGeopoints.add(new LatLng(arr.get(j).latitude, arr
                            .get(j).longitude));
                    Log.i("inside for loop ", "latitude "+listGeopoints.get(j).latitude);
                    Log.i("inside for loop ", "langitude "+listGeopoints.get(j).longitude);
                }
    ...
    private void getLocationAddress(Context context,double latitude, double longitude) {
		...
		try {
			...
			Toast.makeText(context, "some problem with google server",
